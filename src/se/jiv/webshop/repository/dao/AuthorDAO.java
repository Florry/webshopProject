...
{
	private static final Logger LOGGER = Logger.getLogger(AuthorDAO.class.getSimpleName());
		...
		if (isValidAuthor(author, "UPDATE_AUTHOR"))
		{
			...
			try (Connection conn = getConnection())
			{
				...
				try (PreparedStatement pstmt = conn.prepareStatement(sql))
				{
					...
					pstmt.executeUpdate();
					LOGGER.trace("Author updated: " + author);
				}
			} catch (SQLException e)
			{
				WebshopAppException excep = new WebshopAppException(e.getMessage(), this.getClass()
						.getSimpleName(), "UPDATE_AUTHOR");
				LOGGER.error(excep);
				throw excep;
			}
		} else
		...
		if (id > 0)
		{
			...
			try (Connection conn = getConnection())
			{
				...
				try (PreparedStatement pstmt = conn.prepareStatement(sql))
				{
					...
					pstmt.executeUpdate();
					LOGGER.trace("Author of id: " + id + "deleted");
				}
				
			} catch (SQLException e)
			{
				WebshopAppException excep = new WebshopAppException(e.getMessage(), this.getClass()
						.getSimpleName(), "DELETE_AUTHOR");
				LOGGER.error(excep);
				throw excep;
			}
		} else
			...
			try (Connection conn = getConnection())
			{
				...
				try (PreparedStatement pstmt = conn.prepareStatement(sql))
				{
					...
					try (ResultSet rs = pstmt.executeQuery())
					{
						...
						if (rs.next())
						{
							AuthorModel author = new AuthorModel.Builder(rs.getString("firstname"),
									rs.getString("lastname")).dob(rs.getString("dob"))
									.country(rs.getString("country")).build();
							LOGGER.trace("Author get from database: " + author);
							return author;
						}
					}
	...
	public List<AuthorModel> getAuthorsByName(String name) throws WebshopAppException
	{
		if (name != null)
		{
			...
			try (Connection conn = getConnection())
			{
				...
				try (PreparedStatement pstmt = conn.prepareStatement(sql))
				{
					...
					try (ResultSet rs = pstmt.executeQuery())
					{
						...
						while (rs.next())
						{
							authorList.add(new AuthorModel.Builder(rs.getString("firstname"), rs
									.getString("lastname")).dob(rs.getString("dob"))
									.country(rs.getString("country")).build());
						}
						LOGGER.trace("Authors get from database: " + authorList);
						return authorList;
					}
			...
			{
				WebshopAppException excep = new WebshopAppException(e.getMessage(), this.getClass()
						.getSimpleName(), "GET_AUTHORS_BY_NAME");
				LOGGER.error(excep);
				throw excep;
			}
		} else
	...
	private void prepareStatementFromModel(PreparedStatement pstmt, AuthorModel author)
			throws SQLException
	{
			...
			throws WebshopAppException
	{
		if (author == null)
		{
			WebshopAppException excep = new WebshopAppException("author can not be null", this
